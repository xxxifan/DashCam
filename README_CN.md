...
`Downloads/DashCam` 是公开目录，其他支持读取系统下载内容的应用也可以访问。
对记录仪的 TS 视频，保存时可选择保留原始 TS，或无损转封装为更兼容系统相册的 MP4。转换流程只会将 MP4 写入公开目录，并在完成后删除应用缓存中的临时 TS。

> [!WARNING]
> 手机本机未导出的录像仍位于应用专属目录，卸载应用时可能被删除。需要长期保留时请先导出。
